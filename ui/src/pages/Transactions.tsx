...
const Transactions = () => {
  ...
  const fetchTransactions = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch('/transactions', { headers: { 'Authorization': token ? `Bearer ${token}` : '' } });
  ...
  const fetchPayees = React.useCallback(async () => {
    const token = localStorage.getItem('token');
    const res = await fetch('/payees', { headers: { 'Authorization': token ? `Bearer ${token}` : '' } });
    if (res.ok) {
  ...
  const fetchUsers = React.useCallback(async () => {
    const token = localStorage.getItem('token');
    const res = await fetch('/users/all', { headers: { 'Authorization': token ? `Bearer ${token}` : '' } });
    if (res.ok) {
  ...
  const addPayee = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/payees', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': token ? `Bearer ${token}` : '' }, body: JSON.stringify({ name, accountNumber }) });
    if (!res.ok) {
      setBusyAdd(false);
  ...
  const sendPayment = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/payments', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': token ? `Bearer ${token}` : '' }, body: JSON.stringify(payload) });
    if (!res.ok) {
      setBusySend(false);
        ...
        <div className="card">
          ...
          <form onSubmit={sendPayment}>
            <div className="form-group">
              ...
              <select id="fromAccount" aria-label="Choose source account" value={paymentForm.accountId} onChange={e => setPaymentForm({ ...paymentForm, accountId: e.target.value })} required>
            ...
            <div className="form-group">
              ...
              <select id="toPayee" aria-label="Choose a payee" value={paymentForm.payeeId} onChange={e => setPaymentForm({ ...paymentForm, payeeId: e.target.value })} required>
            ...
            <div className="form-group">
              ...
              <input id="amount" type="number" step="0.01" min="0.01" value={paymentForm.amount} onChange={e => setPaymentForm({ ...paymentForm, amount: e.target.value })} required />
            </div>
            <div className="form-group">
              <label htmlFor="desc">Description</label>
              <input id="desc" type="text" value={paymentForm.description} onChange={e => setPaymentForm({ ...paymentForm, description: e.target.value })} placeholder="Optional description" />
            </div>
        ...
        <div className="card">
          ...
          <form onSubmit={addPayee}>
            ...
            <div className="form-group">
              ...
              <input id="payeeName" value={payeeForm.name} onChange={e => setPayeeForm({ ...payeeForm, name: e.target.value })} placeholder="Name (auto-filled if user selected)" />
            </div>
            <div className="form-group">
              <label htmlFor="payeeAcc">Account Number</label>
              <input id="payeeAcc" value={payeeForm.accountNumber} onChange={e => setPayeeForm({ ...payeeForm, accountNumber: e.target.value })} required placeholder="12-digit account number" />
              <small className="small">Enter exact 12 digits.</small>
            </div>
