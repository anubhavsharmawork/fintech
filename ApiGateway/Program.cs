...
try
{
    ...
    var enableOcelot = builder.Configuration.GetValue("ENABLE_OCELOT", true);
    ...
    using (var scope = app.Services.CreateScope())
    {
        ...
        if (db != null)
        {
            try
            {
                ...
                Log.Information("[DB][EF] EF migrations applied for LedgerDbContext");
            }
            catch (Exception mex)
            {
                Log.Error(mex, "[DB][EF] EF migrations failed; will attempt raw ensure/seed");
            }
        }
        else
        {
    ...
    // Ensure DB schema and seed for demo
    if (TryGetConnectionString(app.Configuration, out _))
    {
        await EnsureDatabaseAndSeedAsync(app.Configuration);
    }
    ...
    // Debug endpoints for DB diagnostics
    app.MapGet("/_debug/db", (IConfiguration cfg) =>
    {
        if (TryGetConnectionString(cfg, out var cs))
        {
            var csb = new NpgsqlConnectionStringBuilder(cs);
            return Results.Ok(new { csb.Host, csb.Database, csb.Port, csb.Username, csb.SslMode });
        }
        return Results.NotFound(new { message = "No DB configured" });
    });

    app.MapGet("/_debug/pingdb", async (IConfiguration cfg) =>
    {
        if (!TryGetConnectionString(cfg, out var cs))
            return Results.NotFound(new { message = "No DB configured" });
        try
        {
            ...
            return Results.Ok(new { now, database = dbn, user = usr });
        }
        catch (Exception ex)
        {
            Log.Error(ex, "[DB][Ping] Ping failed");
            return Results.Problem("Ping failed: " + ex.Message, statusCode: 500);
        }
    });

    // Minimal demo auth endpoints + DB-backed register/login
    app.MapPost("/users/login", async (LoginRequest req, IConfiguration cfg, IHttpClientFactory httpFactory, CancellationToken ct) =>
    {
        Log.Information("Login attempt for {Email}", req.Email);

        if (string.Equals(req.Email, "demo", StringComparison.OrdinalIgnoreCase) && req.Password == "demo")
        {
            var token = GenerateJwt(cfg, Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"), "demo");
            Log.Information("Demo login success");
            return Results.Ok(new { message = "Login successful", userId = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa", token });
        }
    ...
    app.MapPost("/users/register", async (RegisterRequest req, IConfiguration cfg, IHttpClientFactory httpFactory, CancellationToken ct) =>
    {
    ...
    app.MapGet("/accounts", async (HttpContext http, IConfiguration cfg) =>
    {
        ...
        catch (Exception ex)
        {
            ...
            return Results.Problem("Failed to load accounts", statusCode: 500);
        }
    }).RequireAuthorization();
    ...
    if (enableOcelot)
    {
        app.MapWhen(ctx =>
        {
            ...
            var isOcelotApiPrefix = path.StartsWith("/users", StringComparison.OrdinalIgnoreCase)
                                    || path.StartsWith("/accounts", StringComparison.OrdinalIgnoreCase)
                                    || path.StartsWith("/transactions", StringComparison.OrdinalIgnoreCase);
...
partial class Program
{
    ...
    // RESTORED: Account number generator -> now 12 digits
    ...
    static async Task EnsureDatabaseAndSeedAsync(IConfiguration cfg)
    {
        // Retry to tolerate cold starts
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++)
        {
            try
            {
                await using var conn = new NpgsqlConnection(GetConnectionString(cfg));
                await conn.OpenAsync();
                Log.Information("[DB][Ensure] Connection opened for ensure/seed (attempt {Attempt})", attempt);

                // Create tables if not exists (distinct names)
                var createUsers = @"CREATE TABLE IF NOT EXISTS users_usvc (
            ""Id"" uuid PRIMARY KEY,
            ...
            ""UpdatedAt"" timestamptz NOT NULL
                ...
                var createAccounts = @"CREATE TABLE IF NOT EXISTS ""LedgerAccounts"" (
            ""Id"" uuid PRIMARY KEY,
            ...
            ""UpdatedAt"" timestamptz NOT NULL
                ...
                var createTransactions = @"CREATE TABLE IF NOT EXISTS ""LedgerTransactions"" (
            ""Id"" uuid PRIMARY KEY,
            ...
            ""CreatedAt"" timestamptz NOT NULL
                ...
                var createPayees = @"CREATE TABLE IF NOT EXISTS ""LedgerPayees"" (
            ""Id"" uuid PRIMARY KEY,
            ""UserId"" uuid NOT NULL,
            ...
            ""CreatedAt"" timestamptz NOT NULL
                ...
                await using (var cmd = new NpgsqlCommand(createUsers, conn)) await cmd.ExecuteNonQueryAsync();
                await using (var cmd = new NpgsqlCommand(createAccounts, conn)) await cmd.ExecuteNonQueryAsync();
                await using (var cmd = new NpgsqlCommand(createTransactions, conn)) await cmd.ExecuteNonQueryAsync();
                await using (var cmd = new NpgsqlCommand(createPayees, conn)) await cmd.ExecuteNonQueryAsync();
                Log.Information("[DB][Ensure] Tables ensured (users_usvc, Ledger*)");

                // Seed demo user
                var demoUserId = Guid.Parse("aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa");
                var existsUserSql = "SELECT 1 FROM users_usvc WHERE \"Email\"='demo' LIMIT 1";
                await using (var cmd = new NpgsqlCommand(existsUserSql, conn))
                {
                    var exists = await cmd.ExecuteScalarAsync();
                    if (exists == null)
                    {
                        var insert = @"INSERT INTO users_usvc (""Id"", ""Email"", ""PasswordHash"", ""FirstName"", ""LastName"", ""IsEmailVerified"", ""CreatedAt"", ""UpdatedAt"")
                               VALUES (@id, 'demo', @ph, 'Demo', 'User', true, NOW(), NOW())";
                        await using var icmd = new NpgsqlCommand(insert, conn);
                        icmd.Parameters.AddWithValue("id", demoUserId);
                        icmd.Parameters.AddWithValue("ph", HashPassword("demo"));
                        await icmd.ExecuteNonQueryAsync();
                        Log.Information("[DB][Seed] Seeded demo user");
                    }
                    else
                    {
                        Log.Information("[DB][Seed] Demo user already exists");
                    }
                }

                // Seed demo accounts in LedgerAccounts
                var checkAcc = "SELECT 1 FROM \"LedgerAccounts\" WHERE \"Id\"='bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'";
                await using (var cmd = new NpgsqlCommand(checkAcc, conn))
                {
                    var exists = await cmd.ExecuteScalarAsync();
                    if (exists == null)
                    {
                        var insert1 = @"INSERT INTO ""LedgerAccounts"" (""Id"", ""UserId"", ""AccountNumber"", ""AccountType"", ""Balance"", ""Currency"", ""CreatedAt"", ""UpdatedAt"")
                                 VALUES ('bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', @uid, '123456789012', 'Checking', 2500.50, 'NZD', NOW(), NOW());";
                        var insert2 = @"INSERT INTO ""LedgerAccounts"" (""Id"", ""UserId"", ""AccountNumber"", ""AccountType"", ""Balance"", ""Currency"", ""CreatedAt"", ""UpdatedAt"")
                                 VALUES ('cccccccc-cccc-cccc-cccc-cccccccccccc', @uid, '098765432109', 'Savings', 10000.00, 'NZD', NOW(), NOW());";
                        await using var cmd1 = new NpgsqlCommand(insert1, conn);
                        cmd1.Parameters.AddWithValue("uid", demoUserId);
                        await cmd1.ExecuteNonQueryAsync();
                        await using var cmd2 = new NpgsqlCommand(insert2, conn);
                        cmd2.Parameters.AddWithValue("uid", demoUserId);
                        await cmd2.ExecuteNonQueryAsync();
                        Log.Information("[DB][Seed] Seeded demo LedgerAccounts");
                    }
                };

                // Seed demo transactions in LedgerTransactions
                var checkTrx = "SELECT 1 FROM \"LedgerTransactions\" WHERE \"Id\"='dddddddd-dddd-dddd-dddd-dddddddddddd'";
                await using (var cmd = new NpgsqlCommand(checkTrx, conn))
                {
                    var exists = await cmd.ExecuteScalarAsync();
                    if (exists == null)
                    {
                        var ins = @"INSERT INTO ""LedgerTransactions"" (""Id"", ""AccountId"", ""UserId"", ""Amount"", ""Currency"", ""Type"", ""Description"", ""CreatedAt"") VALUES
('dddddddd-dddd-dddd-dddd-dddddddddddd','bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',@uid,100.00,'NZD','credit','Salary deposit', NOW() - INTERVAL '1 day'),
('eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee','bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb',@uid,50.00,'NZD','debit','Grocery shopping', NOW() - INTERVAL '2 days'),
('ffffffff-ffff-ffff-ffff-ffffffffffff','cccccccc-cccc-cccc-cccc-cccccccccccc',@uid,500.00,'NZD','credit','Transfer from checking', NOW() - INTERVAL '3 days');";
                        await using var icmd = new NpgsqlCommand(ins, conn);
                        icmd.Parameters.AddWithValue("uid", demoUserId);
                        await icmd.ExecuteNonQueryAsync();
                        Log.Information("[DB][Seed] Seeded demo LedgerTransactions");
                    }
                };

                // Seed demo payees in LedgerPayees
                var checkPy = "SELECT 1 FROM \"LedgerPayees\" WHERE \"Id\"='gggggggg-gggg-gggg-gggg-gggggggggggg'";
                await using (var cmd = new NpgsqlCommand(checkPy, conn))
                {
                    var exists = await cmd.ExecuteScalarAsync();
                    if (exists == null)
                    {
                        var insert = @"INSERT INTO ""LedgerPayees"" (""Id"", ""UserId"", ""Name"", ""AccountNumber"", ""CreatedAt"")
                                 VALUES ('gggggggg-gggg-gggg-gggg-gggggggggggg', @uid, 'Demo Payee', 'DEMO1234567890', NOW());";
                        await using var icmd = new NpgsqlCommand(insert, conn);
                        icmd.Parameters.AddWithValue("uid", demoUserId);
                        await icmd.ExecuteNonQueryAsync();
                        Log.Information("[DB][Seed] Seeded demo LedgerPayees");
                    }
                };
            ...
            {
                Log.Warning(ex, "[DB][Ensure] Database ensure/seed failed (attempt {Attempt}/{Max})", attempt, maxAttempts);
                if (attempt == maxAttempts) break;
                await Task.Delay(TimeSpan.FromSeconds(Math.Pow(2, attempt))); // 2,4,8,16
            }
        }
    }

    // Ensure default account for a user (LedgerAccounts)
    ...
    // Get accounts from LedgerAccounts
    ...
    // Get transactions from LedgerTransactions
    ...
    // CreateTransactionAsync operates on Ledger* tables
    static async Task<TransactionResponse> CreateTransactionAsync(IConfiguration cfg, Guid userId, Guid accountId, decimal amount, string currency, string type, string description)
    {
        ...
        await using (var checkCmd = new NpgsqlCommand(checkSql, conn, (NpgsqlTransaction)tx))
        {
            ...
            var existingCurrency = r.GetString(1);
        ...
        return new TransactionResponse(tid, accountId, amount, currency, type, description, DateTime.UtcNow);
    }
    ...
    // In-memory user model + store used only when DB is not configured and no remote USERS_SERVICE_URL is set
